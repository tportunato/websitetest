...
/* Module scope, deliberately: this survives a route change but NOT a reload.
   Landing unmounts when you go to a content page, so without a flag the veil
   would replay every time you came back. sessionStorage was wrong the other
   way - it survives a refresh too, so the veil played once per tab and never
   again. This gives what it should: the front door on a real page load,
   including a refresh of the home tab, and nothing on in-app navigation. */
...
export default function Landing() {
  /* Arrival moment: branded veil on a real page load, skipped for
     ...
     THE VEIL LIFTS WHEN THE WORDMARK REPORTS ITSELF FINISHED, not on a timer.
     ...
     the last stroke was still travelling. It used to lift at 1750ms and nobody
     ...
     takes 600ms to clear. The fallback is a safety net only - if the draw ever
     fails to report, nobody is left stuck behind the veil. */
